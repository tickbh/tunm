...
impl LuaEngine {
    ...
    pub fn execute_lua(&mut self) -> bool {
        ...
        for elem in temp_list {
            let _ = match elem {
                ...
                LuaElem::DbResult(cookie, ret, err_msg, net_msg) => self.execute_db_result(cookie, ret, err_msg, net_msg),
                LuaElem::RedisResult(cookie, result) => self.execute_redis_result(cookie, result),
                LuaElem::NewConnection(cookie, new_fd, client_ip, server_port) => self.execute_new_connect(cookie, new_fd, client_ip, server_port),
    ...
    pub fn apply_new_connect(&mut self, cookie : u32, new_fd : i32, client_ip : String, server_port : u16) {
        let _guard = self.mutex.lock().unwrap();
    ...
    pub fn apply_db_result(&mut self, cookie : u32, ret : i32, err_msg : Option<String>, net_msg : Option<NetMsg>) {
        let _guard = self.mutex.lock().unwrap();
    ...
    pub fn execute_new_connect(&mut self, cookie : u32, new_fd : i32, client_ip : String, server_port : u16) -> i32 {
    ...
    pub fn execute_db_result(&mut self, cookie : u32, ret : i32, err_msg : Option<String>, net_msg : Option<NetMsg>) -> i32 {
        if ret != 0 {
            self.lua.exec_func3("msg_db_result", cookie, ret, err_msg.unwrap_or("err msg detail miss".to_string()))
        } else {
            if net_msg.is_some() {
    ...
    pub fn execute_redis_result(&mut self, cookie : u32, result : Option<RedisResult<Value>>) -> i32 {
        if result.is_none() {
            ...
            self.lua.exec_func2("msg_redis_result", cookie, RedisWrapperResult(result.unwrap()))
        }
    }
    ...
    pub fn execute_message(&mut self, fd : i32, mut net_msg : NetMsg) -> i32 {
        ...
        self.lua.exec_func3("global_dispatch_command", fd, net_msg.get_pack_name().clone(), net_msg)
    }
    ...
    pub fn execute_args_func(&mut self, func : String, args : Vec<String>) -> i32 {
        match args.len() {
            ...
            6 => self.lua.exec_func6(func, &*args[0], &*args[1], &*args[2], &*args[3], &*args[4], &*args[5]),
            7 => self.lua.exec_func7(func, &*args[0], &*args[1], &*args[2], &*args[3], &*args[4], &*args[5], &*args[6]),
            _ => -1,
        }
    }
