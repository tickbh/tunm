...
impl TelnetUtils {
    ...
    {
        if client.records.len() > 1 && String::from_utf8_lossy(&client.records[0]) == "td" && String::from_utf8_lossy(&client.records[1]) == "td" {
            return true;
        }
        return false;
    }
    ...
    pub fn login(&mut self, fd : i32, bytes : &[u8]) {
        ...
        for (i, b) in bytes.iter().enumerate() {
            ...
            //接收到TAB键消息
                ...
                //如果当前为插入状态，将该字符插入消息字符串中，并发送光标之后的内容给客户端，然后再将光标移回来
                client.data.push(*b);
                if !client.benterpwd {
                    let _ = client.tcp.write(&client.data[client.ipos as usize..]);
                    for _ in 0 .. (client.data[client.ipos as usize..].len() - 1) {
    ...
    pub fn update_data(&mut self, fd : i32, bytes : &[u8]) -> i32 {
        ...
        for (i, b) in bytes.iter().enumerate() {
            ...
            if b == &13 {
                ...
                LuaEngine::instance().apply_exec_string(LuaEngine::convert_excute_string(String::from_utf8_lossy(&client.data).to_string()));
                client.data.clear();
                client.ircdnum = -1;
                client.ipos = 0;
                let _ = client.tcp.write(self.prompt.as_bytes());
            ...
            //接收到方向键或插入键
                ...
                //方向键向右
                    ...
                    //发送光标所处位置的字符，让光标向前一格
                    let _ = client.tcp.write(&client.data[client.ipos as usize..client.ipos as usize + 1]);
                    client.ipos += 1;
                }
    ...
    fn accept_callback(ev : &mut EventLoop, fd : u32, _ : EventFlags, _ : *mut ()) -> i32 {
        ...
        ev.add_event(EventEntry::new(stream.as_fd() as u32, FLAG_READ | FLAG_PERSIST, Some(Self::read_callback), None));

        {
            let _ = stream.write(b"                        ** WELCOME TO TELNET SERVER! **                         \n");
    ...
    pub fn listen(&mut self, addr : &str) {
        ...
        event_loop.add_event(EventEntry::new(listener.as_fd() as u32, FLAG_READ | FLAG_PERSIST, Some(Self::accept_callback), None));
        self.listen_fd = fd;
