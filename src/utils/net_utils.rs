...
impl NetUtils {
    ...
    pub fn lua_read_value(lua : *mut lua_State, config : &Config, index : i32, arg : &str) -> Option<Value> {
        ...
        let value = match t {
            ...
            TYPE_STR => {
                let val : String = unwrap_or!(LuaRead::lua_read_at_position(lua, index), return None);
                Some(Value::from(val))
            }
            TYPE_RAW => {
            ...
            TYPE_MAP => {
                ...
                unsafe {
                    ...
                    while td_rlua::lua_istable(lua, t) && td_rlua::lua_next(lua, t) != 0 {
                        let key : String = unwrap_or!(LuaRead::lua_read_at_position(lua, -2), return None);
                        let field = config.get_field_by_name(&key);
                        if field.is_some() {
                            let sub_val = NetUtils::lua_read_value(lua, config, -1, &*field.unwrap().pattern);
                            if sub_val.is_none() {
                                return None;
                            }
            ...
            TYPE_AU8 | TYPE_AI8 | TYPE_AU16 | TYPE_AI16 | TYPE_AU32 | TYPE_AI32 | TYPE_AFLOAT | TYPE_ASTR | TYPE_ARAW | TYPE_AMAP => {
                ...
                unsafe {
                    ...
                    for i in 1 .. (len + 1) {
                        ...
                        let sub_val = NetUtils::lua_read_value(lua, config, -1, get_name_by_type(t - TYPE_STEP));
                        if sub_val.is_none() {
                            return None;
                        }
    ...
    pub fn lua_convert_value(lua : *mut lua_State, config : &Config, index : i32, args : &Vec<String>) -> Option<Vec<Value>> {
        ...
        let mut val : Vec<Value> = Vec::new();
        for i in 0 .. size {
            let sub_val = NetUtils::lua_read_value(lua, config, i + index, &*args.get(i as usize).unwrap());
            if sub_val.is_none() {
                return None;
            }
            val.push(sub_val.unwrap());
